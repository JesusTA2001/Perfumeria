...
function EditarPerfume({ open, product, onClose, onSave, categories }) {
	...
	useEffect(() => {
		if (open && product) {
			setFormState({
				...
				mililitros: String(product.mililitros || ''),
	...
	const handleSubmit = async (event) => {
		...
		if (!payload.name || Number.isNaN(payload.price) || Number.isNaN(payload.stock) || !payload.mililitros) {
			return;
		}
